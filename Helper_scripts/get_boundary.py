...
def get_bounds(ref_pts, dist):
    ...
    dx_c = np.diff(ref_pts[:, 0])
    dy_c = np.diff(ref_pts[:, 1])
    ...
    x_left = ref_pts[:-1, 0] + d*unit_normal[:, 0]
    y_left = ref_pts[:-1, 1] + d*unit_normal[:, 1]

    x_right = ref_pts[:-1, 0] - d*unit_normal[:, 0]
    y_right = ref_pts[:-1, 1] - d*unit_normal[:, 1]
